...
class TestGPTQQuantizeCommandExitCodes:
    """Tests for GPTQ quantize command exit codes per CLI contract."""
    ...
    def test_exit_code_0_on_success(self, runner: CliRunner, temp_dir: Path) -> None:
        """Test exit code 0 on successful GPTQ quantization."""
        ...
        with patch("llm_quantize.cli.quantize.create_source_model") as mock_source:
            ...
            with patch("llm_quantize.cli.quantize.get_quantizer") as mock_get_quantizer:
                ...
                result = runner.invoke(
                    cli,
                    ["quantize", "test-model", "gptq", "-q", "4bit", "-o", str(temp_dir)],
                )

                assert result.exit_code == 0

    def test_exit_code_2_invalid_quant_level(self, runner: CliRunner) -> None:
        """Test exit code 2 for invalid GPTQ quantization level."""
        with patch("llm_quantize.cli.quantize.create_source_model"):
            result = runner.invoke(
                cli,
                ["quantize", "test-model", "gptq", "-q", "INVALID_QUANT"],
            )
    ...
    def test_exit_code_3_model_not_found(self, runner: CliRunner) -> None:
        ...
        with patch("llm_quantize.cli.quantize.create_source_model") as mock_source:
            ...
            result = runner.invoke(
                cli,
                ["quantize", "nonexistent-model", "gptq", "-q", "4bit"],
            )

            assert result.exit_code == 3


class TestGPTQQuantizeCommandOutput:
    """Tests for GPTQ quantize command output format."""
        ...
        with patch("llm_quantize.cli.quantize.create_source_model") as mock_source:
            ...
            with patch("llm_quantize.cli.quantize.get_quantizer") as mock_get_quantizer:
                ...
                result = runner.invoke(
                    ...
                    [
                        "--format", "json",
                        "quantize", "test-model", "gptq", "-q", "4bit",
                        "-o", str(temp_dir),
                    ],
                )
...
class TestGPTQQuantizeCommandOptions:
    """Tests for GPTQ-specific command options."""
    ...
    def test_group_size_option(self, runner: CliRunner, temp_dir: Path) -> None:
        """Test --group-size option for GPTQ."""
        from llm_quantize.models import QuantizedModel
        ...
        with patch("llm_quantize.cli.quantize.create_source_model") as mock_source:
            ...
            with patch("llm_quantize.cli.quantize.get_quantizer") as mock_get_quantizer:
                ...
                assert result.exit_code == 0
                # Verify group size was passed to config
                mock_quantizer_class.assert_called_once()
                call_kwargs = mock_quantizer_class.call_args.kwargs
                config = call_kwargs.get("config")
                assert config.group_size == 64

    def test_calibration_data_option_gptq(self, runner: CliRunner, temp_dir: Path) -> None:
        """Test --calibration-data option for GPTQ."""
        from llm_quantize.models import QuantizedModel
...
class TestGPTQQuantizationLevels:
    """Tests for valid GPTQ quantization levels."""
    ...
    @pytest.mark.parametrize("quant_level", ["2bit", "3bit", "4bit", "8bit"])
    def test_valid_gptq_quant_levels(
        self, runner: CliRunner, temp_dir: Path, quant_level: str
    ) -> None:
        """Test valid GPTQ quantization levels are accepted."""
        from llm_quantize.models import QuantizedModel
        ...
        with patch("llm_quantize.cli.quantize.create_source_model") as mock_source:
            ...
            with patch("llm_quantize.cli.quantize.get_quantizer") as mock_get_quantizer:
                ...
                result = runner.invoke(
                    cli,
                    [
                        "quantize", "test-model", "gptq", "-q", quant_level,
                        "-o", str(temp_dir),
                    ],
                )
